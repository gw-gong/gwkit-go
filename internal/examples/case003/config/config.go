...
func NewConfig(localConfigOption *hot_cfg.LocalConfigOption) (config *Config, err error) {
	config = &Config{}
	config.BaseConfigCapable, err = hot_cfg.NewLocalBaseConfigCapable(localConfigOption)
	return config, err
}
